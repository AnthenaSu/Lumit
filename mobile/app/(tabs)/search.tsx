...
export default function Search() {
  ...
  // Static search bar bottom in content ≈ 170px, floating bar at top:70
  // So bars touch when scrollY ≈ 100. Start fading at 120, fully gone at 70.
  ...
  const handleScroll = (e: any) => {
    ...
    if (!scrollingUp) {
      // Scrolling DOWN: fade out immediately
      isFloatVisible.current = false;
      Animated.timing(floatAnim, {
        toValue: 0,
        duration: 250,
        useNativeDriver: true,
      }).start(() => setFloatMounted(false));
    } else if (y >= FADE_START) {
      // Scrolling UP, far from top: fully visible
      floatAnim.setValue(1);
    } else if (y <= FADE_END) {
      // Scrolling UP, reached top: hide instantly
      isFloatVisible.current = false;
      floatAnim.setValue(0);
      setFloatMounted(false);
    } else {
      // Scrolling UP, near top: opacity tied to position
        ...
        >
          <Pressable onPress={goToSearch}>
            <BlurView
              intensity={70}
              tint="light"
              style={styles.floatingBarBlur}
            >
              <View style={styles.floatingBarGlass} pointerEvents="none" />
              <View
                style={[styles.searchBarWrap, { margin: 4 }]}
                pointerEvents="none"
              >
                <Text style={styles.searchPlaceholder}>
                  search users, items ...
